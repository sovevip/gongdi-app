...
class MemoryDatabase {
  ...
  int insertAttendance(Map<String, dynamic> a) {
    final ei = _attendance.indexWhere((x) => x['worker_id'] == a['worker_id'] && x['date'] == a['date']);
    if (ei != -1) { _attendance[ei] = {'id': _attendance[ei]['id'], ...a}; return 1; }
    final id = _attendanceIdCounter++; _attendance.add({'id': id, ...a}); return id;
  }
  ...
  Map<String, dynamic> getAttendanceStats(int wid, String ms, String me) {
    final pc = _attendance.where((a) => a['worker_id'] == wid && a['date'] != null && a['date'].compareTo(ms) >= 0 && a['date'].compareTo(me) <= 0 && a['is_present'] == 1).length;
    final ac = _attendance.where((a) => a['worker_id'] == wid && a['date'] != null && a['date'].compareTo(ms) >= 0 && a['date'].compareTo(me) <= 0 && a['is_present'] == 0).length;
    final lc = _attendance.where((a) => a['worker_id'] == wid && a['date'] != null && a['date'].compareTo(ms) >= 0 && a['date'].compareTo(me) <= 0 && a['is_present'] == 2).length;
    final oh = _attendance.where((a) => a['worker_id'] == wid && a['date'] != null && a['date'].compareTo(ms) >= 0 && a['date'].compareTo(me) <= 0 && a['is_present'] == 1).fold(0.0, (s, a) => s + ((a['overtime_hours'] as num?)?.toDouble() ?? 0));
    return {'present': pc, 'absent': ac, 'leave': lc, 'overtime_hours': oh};
  }

  Map<String, dynamic> getCumulativeAttendanceStats(int wid) {
    final all = _attendance.where((a) => a['worker_id'] == wid);
    return {'present': all.where((a) => a['is_present'] == 1).length, 'absent': all.where((a) => a['is_present'] == 0).length, 'leave': all.where((a) => a['is_present'] == 2).length, 'overtime_hours': all.where((a) => a['is_present'] == 1).fold(0.0, (s, a) => s + ((a['overtime_hours'] as num?)?.toDouble() ?? 0))};
  }
